...
fn test_axes_2d_numbins() {
    ...
    assert_eq!(AxesTuple::from((x, y)).numbins(), (5 + 2) * (4 + 2))
}
...
fn test_axes_2d_iter() {
    ...
    let actual: Vec<_> = AxesTuple::from((x, y)).iter().collect();
...
fn test_axes_2d_index() {
    ...
    let xy = AxesTuple::from((x.clone(), y.clone()));
    let actual = vec![
        xy.index(&("A", "C")),
        xy.index(&("B", "C")),
        xy.index(&("A", "D")),
...
fn test_axes_3d_iter() {
    ...
    let actual: Vec<_> = AxesTuple::from((x, y, z)).iter().collect();
    let new = SingleValuedBinRange::new;
    let expected = vec![
...
fn test_axes_3d_index() {
    ...
    let y = CategoryNoFlow::new(vec!["y0", "y1"]);
    let z = CategoryNoFlow::new(vec!["z0", "z1"]);
    let xyz = AxesTuple::from((x.clone(), y.clone(), z.clone()));
    let actual = vec![
        xyz.index(&("x0", "y0", "z0")),
        xyz.index(&("x1", "y0", "z0")),
        xyz.index(&("x0", "y1", "z0")),
